...
- `mdqlPreview/Resources/mermaid.min.js` — Vendored Mermaid runtime (pinned version + sha256, upgraded only via `scripts/fetch-mermaid.sh`; never loaded from a CDN — the preview must stay offline-deterministic for the screenshot/pr-preview path). `MarkdownRenderer.mermaidScriptTag(for:)` inlines it into the HTML only when the body has a `language-mermaid` fence, with `</script` escaped so it can't break out of its tag. The in-page `__mdqlRenderDiagrams` hook (defined in `wrapInHTMLDocument`, re-invoked after FileWatcher innerHTML swaps) renders fences as SVG; failed diagrams keep their source visible with an error note.
